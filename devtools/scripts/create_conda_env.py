...
try:
    ...
    # typ="safe" avoids odd typing on output
...
try:
    for dep_index, dep_value in enumerate(yaml_script['dependencies']):
        if re.match('python([ ><=*]+[0-9.*]*)?$',
                    dep_value):  # Match explicitly 'python' and its formats
    ...
    # Ensure the python version is added in. Even if the code does not need
    # it, we assume the env does
...
with temp_cd():
    ...
    sp.call("{} env create -n {} -f {}".format(conda_path,
                                               args.name, temp_file_name), shell=True)

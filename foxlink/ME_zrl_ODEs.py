...
from .ME_frl_helpers import (boltz_fact_frl, weighted_boltz_fact_frl,
                             fast_frl_src_full_kl)
...
@njit
def du1_dt_frl(r12, u1, u2, P1, mu11, a1, b, ks, grot1):
...
@njit
def du2_dt_frl(r12, u1, u2, P2, mu11, a2, b, ks, grot2):
...
def drho_dt_frl(rho, rsqr, a1, a2, b, vo, fs, ko, c, ks, beta, L1, L2, q=None):
    ...
    # Partition function
    if q is None:
        q, e = dblquad(boltz_fact_frl, -.5 * L1, .5 * L1,
                       lambda s2: -.5 * L2, lambda s2: .5 * L2,
                       args=[rsqr, a1, a2, b, ks, beta])
    elif q == 'fast':
        q = fast_frl_src_full_kl(L1, L2, rsqr, a1, a2, b, ks, beta, k=0, l=0)

    return ko * (c * q - rho)


def dP1_dt_frl(rho, P1, P2, rsqr, a1, a2, b, vo, fs,
    ...
    # Partition function
    if q10 is None:
        q10, e = dblquad(weighted_boltz_fact_frl, -.5 * L1, .5 * L1,
                         lambda s2: -.5 * L2, lambda s2: .5 * L2,
                         args=[1, 0, rsqr, a1, a2, b, ks, beta],)
    elif q10 == 'fast':
        # q10 = fast_frl_src_full_kl(L1, L2, rsqr, a1, a2, b, ks, beta, k=1, l=0)
        # Make coordinate transformation
        q10 = fast_frl_src_full_kl(
...
def dP2_dt_frl(rho, P1, P2, rsqr, a1, a2, b, vo,
    ...
    # Partition function
    if q01 is None:
        q01, e = dblquad(weighted_boltz_fact_frl, -.5 * L1, .5 * L1,
                         lambda s2: -.5 * L2, lambda s2: .5 * L2,
                         args=[0, 1, rsqr, a1, a2, b, ks, beta])
    elif q01 == 'fast':
        q01 = fast_frl_src_full_kl(L1, L2, rsqr, a1, a2, b, ks, beta, k=0, l=1)
    # Characteristic walking rate
    kappa = vo * ks / fs
...
def dmu11_dt_frl(rho, P1, P2, mu11, mu20, mu02, rsqr,
    ...
    # Partition function
    if q11 is None:
        q11, e = dblquad(weighted_boltz_fact_frl, -.5 * L1, .5 * L1,
                         lambda s2: -.5 * L2, lambda s2: .5 * L2,
                         args=[1, 1, rsqr, a1, a2, b, ks, beta])
    elif q11 == 'fast':
        q11 = fast_frl_src_full_kl(L1, L2, rsqr, a1, a2, b, ks, beta, k=1, l=1)
    # Characteristic walking rate
    kappa = vo * ks / fs
...
def dmu20_dt_frl(rho, P1, P2, mu11, mu20, mu02, rsqr,
    ...
    # Partition function
    if q20 is None:
        q20, e = dblquad(weighted_boltz_fact_frl, -.5 * L1, .5 * L1,
                         lambda s2: -.5 * L2, lambda s2: .5 * L2,
                         args=[2, 0, rsqr, a1, a2, b, ks, beta])
    elif q20 == 'fast':
        # q20 = fast_frl_src_full_kl(L1, L2, rsqr, a1, a2, b, ks, beta, k=2, l=0)
        # Make coordinate transformation
        q20 = fast_frl_src_full_kl(
...
def dmu02_dt_frl(rho, P1, P2, mu11, mu20, mu02, rsqr,
    ...
    # Partition function
    if q02 is None:
        q02, e = dblquad(weighted_boltz_fact_frl, -.5 * L1, .5 * L1,
                         lambda s2: -.5 * L2, lambda s2: .5 * L2,
                         args=[0, 2, rsqr, a1, a2, b, ks, beta])
    elif q02 == 'fast':
        q02 = fast_frl_src_full_kl(L1, L2, rsqr, a1, a2, b, ks, beta, k=0, l=2)
    # Characteristic walking rate
    kappa = vo * ks / fs

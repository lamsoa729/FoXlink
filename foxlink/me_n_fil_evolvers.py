...
from .me_zrl_helpers import (
    ...
    get_unbound_and_zrl_xl_moments_for_ij,
)

from .me_zrl_xl_odes import (
...
def me_evolver_nfil_crosslink(sol, fric_coeff_arr, params):
    ...
    derivs = np.zeros((n_fils * 7 + n_fils * (n_fils - 1) * 2))
    ...
    for i in range(n_fils):
        ...
        for j in range(i + 1, n_fils):
            ...
            mu_kl = get_zrl_xl_moments_for_ij(sol, i, j, n_fils)
            ...
            # Moment evolution
            dmu_kl = calc_zrl_xl_moment_derivs(mu_kl, q_arr, co, ko)
...
def me_evolver_nfil_finite_crosslink(sol, fric_coeff_arr, params):
    """!Calculate all time derivatives necessary to solve the moment expansion
    evolution of the Fokker-Planck equation of zero rest length (zrl) crosslinkers
    bound to moving rods. d<var> is the time derivative of corresponding variable.
    Currently only works for static crosslinkers.
    ...
    for i in range(n_fils):
        ...
        for j in range(i + 1, n_fils):
            ...
            # mu_kl = [mu^{00}, mu^{01}, mu^{10}, mu^{11}]
            n_unbound, mu_kl = get_unbound_and_zrl_xl_moments_for_ij(sol, i, j, n_fils)
            ...
            # Moment evolution
            dn_unbound, dmu_kl = calc_zrl_unbound_and_xl_moment_derivs(
                n_unbound, mu_kl, q_arr, ko
            )

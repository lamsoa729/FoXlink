...
Description:
    me = moment expansion solvers
"""

import numpy as np
from scipy.integrate import dblquad
from .me_helpers import dr_dt, convert_sol_to_geom, rod_geom_derivs
from .me_gen_helpers import (
    weighted_boltz_fact_gen,
    boltz_fact_gen,
    avg_force_gen,
    avg_force_gen_2ord,
    avg_torque_gen_ij,
        ...
        for i in range(n + 1):
            src_terms += [
                ...
                * dblquad(
                    ...
                    args=[
                        n - i,
                        i,  # Specify powers, e.g. n=2 would give moments mu20, mu11, and mu02
                        rsqr,
                        a_ij,
                        a_ji,
                        b,
...
def prep_me_evolver_gen_2ord(sol, params):
    ...
    q00 = (
        ...
        * dblquad(
            ...
            lambda s_j: -0.5 * L_j,
            lambda s_j: 0.5 * L_j,
            args=[rsqr, a_ij, a_ji, b, ks, ho, beta],
            epsrel=1e-5,
        )[0]
    )  # only want val, not error
    q10 = (
        co
        * dblquad(
            weighted_boltz_fact_gen,
            ...
            lambda s_j: -0.5 * L_j,
            lambda s_j: 0.5 * L_j,
            args=[1, 0, rsqr, a_ij, a_ji, b, ks, ho, beta],
            epsrel=1e-5,
        )[0]
    )
    q01 = (
        ...
        * dblquad(
            ...
            lambda s_j: -0.5 * L_j,
            lambda s_j: 0.5 * L_j,
            args=[0, 1, rsqr, a_ij, a_ji, b, ks, ho, beta],
            epsrel=1e-5,
        )[0]
    )
    q11 = (
        ...
        * dblquad(
            ...
            lambda s_j: -0.5 * L_j,
            lambda s_j: 0.5 * L_j,
            args=[1, 1, rsqr, a_ij, a_ji, b, ks, ho, beta],
            epsrel=1e-5,
        )[0]
    )
    q20 = (
        ...
        * dblquad(
            ...
            lambda s_j: -0.5 * L_j,
            lambda s_j: 0.5 * L_j,
            args=[2, 0, rsqr, a_ij, a_ji, b, ks, ho, beta],
            epsrel=1e-5,
        )[0]
    )
    q02 = (
        ...
        * dblquad(
            ...
            lambda s_j: -0.5 * L_j,
            lambda s_j: 0.5 * L_j,
            args=[0, 2, rsqr, a_ij, a_ji, b, ks, ho, beta],
            epsrel=1e-5,
        )[0]
    )
    return (
...
def me_evolver_gen_2ord(sol, fric_coeff, params):
    ...
    (
        ...
        q20,
        q02,
    ) = prep_me_evolver_gen_2ord(sol, params)

    # Get average force of crosslinkers on rod_j
    f_ij = avg_force_gen_2ord(
        r_ij, u_i, u_j, rsqr, a_ij, a_ji, b, mu00, mu10, mu01, mu11, mu20, mu02, ks, ho
    )
    # Evolution of rod positions
    dr_i = dr_dt(-1.0 * f_ij, u_i, fric_coeff[0], fric_coeff[1])
    dr_j = dr_dt(f_ij, u_j, fric_coeff[3], fric_coeff[4])
    # Evolution of orientation vectors
    du_i = du_dt_gen_2ord(
        r_ij, u_i, u_j, rsqr, a_ij, a_ji, b, mu10, mu11, mu20, ks, ho, fric_coeff[2]
    )
    du_j = du_dt_gen_2ord(
        -1.0 * r_ij,
        u_j,
    ...
    dmu11 = dmu11_dt_gen_2ord(
        rsqr, a_ij, a_ji, b, mu10, mu01, mu11, mu20, mu02, ko, vo, fs, ks, ho, q=q11
    )
    dmu20 = dmu20_dt_gen_2ord(
        rsqr, a_ij, a_ji, b, mu10, mu11, mu20, ko, vo, fs, ks, ho, q=q20
    )
    dmu02 = dmu20_dt_gen_2ord(
        rsqr,
        a_ji,
        a_ij,
        b,  # ij->ji
...
def me_evolver_pass_gen(sol, fric_coeff, params):
    ...
    beta = params["beta"]
    # Get variables needed to solve ODE
    (r_ij, u_i, u_j, rsqr, a_ij, a_ji, b, moments, src_terms) = (
        prep_me_evolver_gen_n_ord(sol, params)
    )

    # Get average force of crosslinkers on rod_j
    f_ij = avg_force_gen(r_ij, u_i, u_j, rsqr, a_ij, a_ji, b, ks, ho, *moments[:10])
    t_ij = avg_torque_gen_ij(r_ij, u_i, u_j, rsqr, a_ij, a_ji, b, ks, ho, *moments)
    t_ji = avg_torque_gen_ji(r_ij, u_i, u_j, rsqr, a_ij, a_ji, b, ks, ho, *moments)

    # Evolution of rod positions
    # dr_i = dr_dt(-1. * f_ij, u_i, fric_coeff[0], fric_coeff[1])
    # dr_j = dr_dt(f_ij, u_j, fric_coeff[3], fric_coeff[4])
    (dr_i, dr_j, du_i, du_j) = rod_geom_derivs(f_ij, t_ji, t_ij, u_i, u_j, fric_coeff)

    dmu_lst = []
    for qkl, mukl in zip(moments, src_terms):
        dmu_lst += [ko * qkl - ko * mukl]

    dsol = np.concatenate((dr_i, dr_j, du_i, du_j, dmu_lst))
    # Check to make sure all values are finite
    if not np.all(np.isfinite(dsol)):
        raise RuntimeError(
            "Infinity or NaN thrown in ODE solver derivatives. Current derivatives",

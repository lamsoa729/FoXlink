...
def get_max_min_ends(r_i, r_j, u_i, u_j, L_i, L_j):
    ...
    """
    return [
        np.amax(0.5 * L_i * u_i + r_i),
        np.amin(0.5 * L_i * u_i + r_i),
        np.amax(-0.5 * L_i * u_i + r_i),
        np.amin(-0.5 * L_i * u_i + r_i),
        np.amax(0.5 * L_j * u_j + r_j),
        np.amin(0.5 * L_j * u_j + r_j),
        np.amax(-0.5 * L_j * u_j + r_j),
        np.amin(-0.5 * L_j * u_j + r_j),
    ]
...
def draw_rod(ax, r_vec, u_vec, L, rod_diam, color="tab:green", tip_color="b", x=1, y=2):
    ...
    """
    line = LineDataUnits(
        (r_vec[x] - 0.5 * L * u_vec[x], r_vec[x] + 0.5 * L * u_vec[x]),
        (r_vec[y] - 0.5 * L * u_vec[2], r_vec[y] + 0.5 * L * u_vec[y]),
        linewidth=rod_diam,
    ...
    tip = Circle(
        (r_vec[x] + 0.5 * L * u_vec[x], r_vec[y] + 0.5 * L * u_vec[y]),
        0.5 * rod_diam,
        color=tip_color,
        zorder=3,
    )
...
def draw_xlink(ax, e_i, e_j, lw=10, color="k", alpha=0.5):
...
def draw_moment_rod(ax, r_vec, u_vec, L, rod_diam, mu00, mu10, mu20, num_max=50):
    ...
    """
    cb = mpl.cm.ScalarMappable(mpl.colors.Normalize(0, num_max), "viridis")
    draw_rod(ax, r_vec, u_vec, L, rod_diam)
    scaled_mu10 = mu10 / mu00 if mu00 else 0
    mu10_loc = RegularPolygon(
        (r_vec[1] + scaled_mu10 * u_vec[1], r_vec[2] + scaled_mu10 * u_vec[2]),
        5,
        rod_diam,
        color=cb.to_rgba(mu00),
        zorder=4,
    )
    variance = (mu20 / mu00) - (scaled_mu10**2) if mu00 > 1e-3 else 0.0

    sigma_dist = np.sqrt(variance) if variance >= 1e-3 else 0.0
    ...
    sigma_bar = FancyArrowPatch(
        ...
        arrowstyle=ArrowStyle(
            "|-|",
            widthA=convert_size_units(0.5 * rod_diam, ax),
            widthB=convert_size_units(0.5 * rod_diam, ax),
        ),
        zorder=3,
    )
...
def graph_vs_time(ax, time, y, n=-1, color="b", fillstyle="full"):
    ...
    """
    s = ax.plot(time[:n], y[:n], c=color, marker="o", fillstyle=fillstyle, linestyle="")
    return s
...
def graph_2d_rod_diagram(ax, anal, n=-1):
    ...
    if hasattr(anal, "phi_arr") and not hasattr(anal, "R1_vec"):
        ...
        max_x = max_x * 1.25 if max_x > 0 else 0.75 * max_x
        min_x = min(x_ends + y_ends)
        min_x = min_x * 1.25 if min_x < 0 else 0.75 * min_x
...
def graph_2d_rod_pde_diagram(ax, anal, n=-1, scale=50):
    ...
    s_i = np.arange(-0.5 * (L_i + rod_diam), 0.5 * (L_i + rod_diam), rod_diam)
    s_j = np.arange(-0.5 * (L_j + rod_diam), 0.5 * (L_j + rod_diam), rod_diam)
    ...
    for index, val in np.ndenumerate(xl_distr_coarse):
        ...
        # print(e_i, e_j)
        draw_xlink(ax, e_i, e_j, color="r", alpha=np.clip(val * scale / (a * b), 0, 1))
...
def me_graph_min_data_2d(fig, axarr, n, me_anal):
    ...
    axarr[1].set_xlabel("Head distance from \n center of fil$_i$ $s_i$ (nm)")
    axarr[1].set_ylabel("Head distance from \n center of fil$_j$ $s_j$ (nm)")

    if me_anal.init_flag:
        axarr[0].set_aspect(1.0)
        axarr[1].set_aspect(1.0)
        cbar = fig.colorbar(cb, ax=axarr[0])
        cbar.set_label(r"Motor number $\langle N_{i,j} \rangle$")
        cbar1 = fig.colorbar(cb1, ax=axarr[1])
        cbar1.set_label(r"Reconstructed motor density $\psi_{i,j}$")
    me_anal.init_flag = False

    axarr[0].text(
        0.05,
        0.90,
        "Time = {:.2f} sec".format(me_anal.time[n]),
...
def me_graph_all_data_2d(fig, axarr, n, me_anal):
    ...
    axarr[1].set_ylim(np.amin(me_anal.dR_arr), np.amax(me_anal.dR_arr))
    ...
    axarr[2].set_ylim(np.nanmin(me_anal.phi_arr), np.nanmax(me_anal.phi_arr))

    axarr[3].set_xlabel(r"Time (sec)")
    axarr[3].set_ylabel(r"Motor number")
    axarr[3].set_xlim(left=0, right=me_anal.time[-1])
    axarr[3].set_ylim(np.amin(me_anal.mu00), np.amax(me_anal.mu00))
    ...
    graph_vs_time(axarr[4], me_anal.time, me_anal.mu10, n, color="tab:green")
    graph_vs_time(axarr[4], me_anal.time, me_anal.mu01, n, color="tab:purple")

    # Graph second moments of crosslinker distribution
    graph_vs_time(axarr[5], me_anal.time, me_anal.mu11, n, color="b")
    graph_vs_time(axarr[5], me_anal.time, me_anal.mu20, n, color="tab:green")
    graph_vs_time(axarr[5], me_anal.time, me_anal.mu02, n, color="tab:purple")
    ...
    # Legend information
    axarr[1].legend(
        [r"$\Delta$R({:.2f}) = {:.1f} nm".format(me_anal.time[n], me_anal.dR_arr[n])]
    )
    axarr[2].legend(
        [r"$\phi$({:.2f}) = {:.1f} rad".format(me_anal.time[n], me_anal.phi_arr[n])]
    )
    axarr[3].legend([r"N({:.2f})={:.1f}".format(me_anal.time[n], me_anal.mu00[n])])
    axarr[4].legend(
        [
            r"$\mu^{{1,0}}$({:.2f}) = {:.1f}".format(me_anal.time[n], me_anal.mu10[n]),
            r"$\mu^{{0,1}}$({:.2f}) = {:.1f}".format(me_anal.time[n], me_anal.mu01[n]),
        ]
    )
    axarr[5].legend(
        [
            r"$\mu^{{1,1}}$({:.2f}) = {:.1f}".format(me_anal.time[n], me_anal.mu11[n]),
            r"$\mu^{{2,0}}$({:.2f}) = {:.1f}".format(me_anal.time[n], me_anal.mu20[n]),
            r"$\mu^{{0,2}}$({:.2f}) = {:.1f}".format(me_anal.time[n], me_anal.mu02[n]),
        ]
    )
    return fig.gca().lines + fig.gca().collections
...
def me_graph_distr_data_2d(fig, axarr, n, me_anal):
    ...
    axarr[3].set_xlim(left=0, right=me_anal.time[-1])
    axarr[3].set_ylim(np.amin(me_anal.dR_arr), np.amax(me_anal.dR_arr))
    ...
    axarr[4].set_xlim(left=0, right=me_anal.time[-1])
    axarr[4].set_ylim(np.nanmin(me_anal.phi_arr), np.nanmax(me_anal.phi_arr))
    ...
    axarr[2].set_xlim(left=0, right=me_anal.time[-1])
    axarr[2].set_ylim(np.amin(me_anal.mu00), np.amax(me_anal.mu00))
    graph_vs_time(axarr[2], me_anal.time, me_anal.mu00, n)
    if me_anal._params["ODE_type"] == "zrl_bvg":
        graph_vs_time(
            axarr[2], me_anal.time, me_anal.mu_kl_eff[:, 0], n, fillstyle="none"
        )
    ...
    graph_vs_time(axarr[5], me_anal.time, me_anal.mu10, n, color="tab:green")
    graph_vs_time(axarr[5], me_anal.time, me_anal.mu01, n, color="tab:purple")
    if me_anal._params["ODE_type"] == "zrl_bvg":
        graph_vs_time(
            axarr[5],
            me_anal.time,
    ...
    graph_vs_time(axarr[8], me_anal.time, me_anal.mu11, n, color="b")
    graph_vs_time(axarr[8], me_anal.time, me_anal.mu20, n, color="tab:green")
    graph_vs_time(axarr[8], me_anal.time, me_anal.mu02, n, color="tab:purple")
    if me_anal._params["ODE_type"] == "zrl_bvg":
        graph_vs_time(
            axarr[8],
            me_anal.time,
...
def pde_graph_all_data_2d(fig, axarr, n, pde_anal):
    ...
    axarr[4].set_ylim(np.amin(pde_anal.torque_arr), np.amax(pde_anal.torque_arr))

    axarr[5].set_xlabel(r"Time (sec)")
    axarr[5].set_ylabel(r"First moments (nm)")
    axarr[5].set_xlim(left=0, right=pde_anal.time[-1])
    axarr[5].set_ylim(
        min(np.amin(pde_anal.mu10), np.amin(pde_anal.mu01)),
    ...
    axarr[6].set_ylim(np.amin(pde_anal.dR_arr), np.amax(pde_anal.dR_arr))

    axarr[7].set_xlabel(r"Time (sec)")
    axarr[7].set_ylabel("Angle between fil \n orientation vectors (rad)")
    axarr[7].set_xlim(left=0, right=pde_anal.time[-1])
    axarr[7].set_ylim(np.nanmin(pde_anal.phi_arr), np.nanmax(pde_anal.phi_arr))

    axarr[8].set_xlabel(r"Time (sec)")
    axarr[8].set_ylabel(r"Second moments (nm$^2$)")
    axarr[8].set_xlim(left=0, right=pde_anal.time[-1])
    axarr[8].set_ylim(
        min(np.amin(pde_anal.mu11), np.amin(pde_anal.mu20), np.amin(pde_anal.mu02)),
        max(np.amax(pde_anal.mu11), np.amax(pde_anal.mu20), np.amax(pde_anal.mu02)),
    )
    ...
    # Graph forces
    graph_vs_time(
        axarr[3], pde_anal.time, pde_anal.force_arr[:, 0], n, color="tab:green"
    )
    graph_vs_time(
        axarr[3], pde_anal.time, pde_anal.force_arr[:, 1], n, color="tab:purple"
    )
    # Graph torques
    graph_vs_time(
        axarr[4], pde_anal.time, pde_anal.torque_arr[:, 0], n, color="tab:green"
    )
    graph_vs_time(
        axarr[4], pde_anal.time, pde_anal.torque_arr[:, 1], n, color="tab:purple"
    )
    # Graph first moments of crosslink distribution
    graph_vs_time(axarr[5], pde_anal.time, pde_anal.mu10, n, color="tab:green")
    graph_vs_time(axarr[5], pde_anal.time, pde_anal.mu01, n, color="tab:purple")
    # Graph rod center separations
    graph_vs_time(axarr[6], pde_anal.time, pde_anal.dR_arr, n)
    # Graph angle between rod orientations
    graph_vs_time(axarr[7], pde_anal.time, pde_anal.phi_arr, n)
    # Graph second moments of crosslinker distribution
    graph_vs_time(axarr[8], pde_anal.time, pde_anal.mu11, n, color="b")
    graph_vs_time(axarr[8], pde_anal.time, pde_anal.mu20, n, color="tab:green")
    graph_vs_time(axarr[8], pde_anal.time, pde_anal.mu02, n, color="tab:purple")

    # Legend information
    axarr[2].legend(["N({:.2f}) = {:.1f}".format(pde_anal.time[n], pde_anal.mu00[n])])
    axarr[3].legend(
        [
            r"F$_i$({:.2f}) = {:.1f}".format(
                pde_anal.time[n], pde_anal.force_arr[n, 0]
            ),
            r"F$_j$({:.2f}) = {:.1f}".format(
                pde_anal.time[n], pde_anal.force_arr[n, 1]
            ),
        ]
    )
    axarr[4].legend(
        [
            r"$T_i$({:.2f}) = {:.1f}".format(
                pde_anal.time[n], pde_anal.torque_arr[n, 0]
            ),
            r"$T_j$({:.2f}) = {:.1f}".format(
                pde_anal.time[n], pde_anal.torque_arr[n, 1]
            ),
        ]
    )
    axarr[5].legend(
        [
            r"$\mu^{{1,0}}$({:.2f}) = {:.1f}".format(
                pde_anal.time[n], pde_anal.mu10[n]
            ),
            r"$\mu^{{0,1}}$({:.2f}) = {:.1f}".format(
                pde_anal.time[n], pde_anal.mu01[n]
            ),
        ]
    )
    axarr[6].legend(
        [r"$\Delta$R({:.2f}) = {:.1f} nm".format(pde_anal.time[n], pde_anal.dR_arr[n])]
    )
    axarr[7].legend(
        [r"$\phi$({:.2f}) = {:.1f} rad".format(pde_anal.time[n], pde_anal.phi_arr[n])]
    )
    axarr[8].legend(
        [
            r"$\mu^{{1,1}}$({:.2f}) = {:.1f}".format(
                pde_anal.time[n], pde_anal.mu11[n]
            ),
            r"$\mu^{{2,0}}$({:.2f}) = {:.1f}".format(
                pde_anal.time[n], pde_anal.mu20[n]
            ),
            r"$\mu^{{0,2}}$({:.2f}) = {:.1f}".format(
                pde_anal.time[n], pde_anal.mu02[n]
            ),
        ]
    )
    return fig.gca().lines + fig.gca().collections
...
def pde_graph_moment_data_2d(fig, axarr, n, pde_anal):
    ...
    axarr[5].set_xlim(left=0, right=pde_anal.time[-1])
    axarr[5].set_ylim(np.amin(pde_anal.force_arr), np.amax(pde_anal.force_arr))

    axarr[6].set_xlabel(r"Time (sec)")
    axarr[6].set_ylabel(r"Motor torque (pN$\cdot$nm)")
    axarr[6].set_xlim(left=0, right=pde_anal.time[-1])
    axarr[6].set_ylim(np.amin(pde_anal.torque_arr), np.amax(pde_anal.torque_arr))
    ...
    graph_vs_time(axarr[2], pde_anal.time, pde_anal.mu00, n)
    # Graph first moments of crosslink distribution
    graph_vs_time(axarr[3], pde_anal.time, pde_anal.mu10, n, color="tab:green")
    graph_vs_time(axarr[3], pde_anal.time, pde_anal.mu01, n, color="tab:purple")
    # Graph second moments of crosslinker distribution
    graph_vs_time(axarr[4], pde_anal.time, pde_anal.mu11, n, color="b")
    graph_vs_time(axarr[4], pde_anal.time, pde_anal.mu20, n, color="tab:green")
    graph_vs_time(axarr[4], pde_anal.time, pde_anal.mu02, n, color="tab:purple")
    # Graph forces
    graph_vs_time(
        axarr[5], pde_anal.time, pde_anal.force_arr[:, 0], n, color="tab:green"
    )
    graph_vs_time(
        axarr[5], pde_anal.time, pde_anal.force_arr[:, 1], n, color="tab:purple"
    )
    # Graph torques
    graph_vs_time(
        axarr[6], pde_anal.time, pde_anal.torque_arr[:, 0], n, color="tab:green"
    )
    graph_vs_time(
        axarr[6], pde_anal.time, pde_anal.torque_arr[:, 1], n, color="tab:purple"
    )
    # Legend information
    axarr[2].legend([r"N({:.2f}) = {:.1f}".format(pde_anal.time[n], pde_anal.mu00[n])])
    axarr[3].legend(
        [
            r"$\mu^{{1,0}}$({:.2f}) = {:.1f}".format(
                pde_anal.time[n], pde_anal.mu10[n]
...
def pde_graph_mts_xlink_distr_2d(fig, axarr, n, pde_anal):
    ...
    # Draw rods
    graph_2d_rod_pde_diagram(axarr[0], pde_anal, n, scale=1.0 / (pde_anal.max_dens_val))
    ...
    axarr[0].text(
        0.05,
        0.90,
        "Time = {:.2f} sec".format(pde_anal.time[n]),
        horizontalalignment="left",
        verticalalignment="bottom",
        transform=axarr[0].transAxes,
    )
...
def pde_graph_recreate_xlink_distr_2d(fig, axarr, n, pde_anal):
    ...
    axarr[1].set_xlabel("Head distance from \n center of fil$_i$ $s_i$ (nm)")
    axarr[1].set_ylabel("Head distance from \n center of fil$_j$ $s_j$ (nm)")
    axarr[2].set_xlabel("Head distance from \n center of fil$_i$ $s_i$ (nm)")
    axarr[2].set_ylabel("Head distance from \n center of fil$_j$ $s_j$ (nm)")

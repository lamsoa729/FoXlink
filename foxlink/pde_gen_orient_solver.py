...
def reparameterize_rods(R1_pos, R2_pos, R1_vec, R2_vec):
    ...
    if not np.isclose(r, 0.0, atol=1e-13):
        rvec = rvec / r
...
class PDEGenOrientSolver(PDESolver):
    ...
    def calcSourceMatrix(self):
        ...
        self.src_mat = make_gen_source_mat(
            self.s1,
            self.s2,
            r,
            a1,
    ...
    def calcTorqueMatrix(self):
        ...
        self.t_mat1 = make_gen_torque_mat(
            -1.0 * np.swapaxes(self.f_mat_dens, 0, 1), self.s1, self.R1_vec
        )
        self.t_mat2 = make_gen_torque_mat(self.f_mat_dens, self.s2, self.R2_vec)
